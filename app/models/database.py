...
class EstadisticasRepository:
    ...
    @staticmethod
    def get_timeline(fecha_desde=None, fecha_hasta=None):
        """Daily ventas / pedidos series for the line chart."""
        conditions = []
        params = {}
        if fecha_desde:
            conditions.append("fecha_pago >= %(desde)s")
    ...
    @staticmethod
    def get_top_productos(fecha_desde=None, fecha_hasta=None, limit=5):
        """Top N products by revenue in the given period."""
        conditions = []
        params = {'limit': limit}
        if fecha_desde:
            conditions.append("fecha_pago >= %(desde)s")
        ...
        prod_filter = "AND producto_nombre IS NOT NULL AND producto_nombre <> ''" if conditions else "WHERE producto_nombre IS NOT NULL AND producto_nombre <> ''"
        query = f"""
            SELECT
                producto_nombre AS producto,
                COUNT(*) AS total_vendidos,
                COALESCE(SUM(precio_total), 0) AS ingresos
            FROM pedidos {where} {prod_filter}
        ...
        with DatabaseManager.get_cursor() as cursor:
            ...
            total_ingresos = sum(float(r['ingresos']) for r in rows) or 1
            return [
                {
                    'producto': r['producto'],
                    'total_vendidos': r['total_vendidos'],
                    'ingresos': round(float(r['ingresos']), 2),
                    'pct': round(float(r['ingresos']) / total_ingresos * 100, 1)
                }
                for r in rows
            ]
    ...
    def get_resumen_operativo():
        """4 operative counts for the Pedidos context bar (single query, very fast)."""
        query = """
            SELECT
                COUNT(*) FILTER (WHERE estado_produccion NOT IN ('Entregado', 'Cancelado'))
                    AS activos,
                COUNT(*) FILTER (WHERE estado_produccion = 'Listo para Envío')
                    AS listos_envio,
                COUNT(*) FILTER (
                ...
                COUNT(*) FILTER (
                    -- 'Recibido' means payment received (paid). Enum has no 'Pagado' value.
                    WHERE estado_pago::text NOT IN ('Recibido', 'Cancelado')
                    AND estado_produccion::text != 'Cancelado'
                ) AS sin_cobrar
            FROM pedidos
        """
        with DatabaseManager.get_cursor() as cursor:
            ...
            row = cursor.fetchone()
            if row:
                return {k: int(row[k] or 0) for k in ('activos', 'listos_envio', 'atrasados', 'sin_cobrar')}
        return {'activos': 0, 'listos_envio': 0, 'atrasados': 0, 'sin_cobrar': 0}
